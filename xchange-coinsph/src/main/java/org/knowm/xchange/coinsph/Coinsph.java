...
public interface Coinsph {
   ...
   * If no symbol is provided, tickers for all symbols will be returned in an array.
   *
   * @return List of ticker information, or single ticker if symbol is provided (API behavior might vary)
   * @throws IOException
   * @throws CoinsphException
   */
  @GET
  @Path("ticker/24hr")
  List<CoinsphTicker> getTicker24hr() throws IOException, CoinsphException; // For all symbols (no symbol query param)
