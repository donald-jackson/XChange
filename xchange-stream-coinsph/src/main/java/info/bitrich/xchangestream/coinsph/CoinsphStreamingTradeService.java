...
public class CoinsphStreamingTradeService implements StreamingTradeService {
  ...
  public Observable<Order> getOrderChanges(CurrencyPair currencyPair, Object... args) {
    // User data streams (executionReport) are typically not per-symbol for subscription in Coins.ph/Binance style.
    ...
    // Ensure user data stream is started (this might be done in CoinsphStreamingService or Exchange)
    ...
    return service
        ...
        .filter(report -> currencyPair == null || CoinsphStreamingAdapters.getCurrencyPairFromSymbol(report.getSymbol()).equals(currencyPair)) // To be created
  ...
  public Observable<UserTrade> getUserTrades(CurrencyPair currencyPair, Object... args) {
    ...
    return service
        ...
        .filter(report -> currencyPair == null || CoinsphStreamingAdapters.getCurrencyPairFromSymbol(report.getSymbol()).equals(currencyPair))
        .map(CoinsphStreamingAdapters::adaptUserTrade);
  }
  
  // Note: getOrderChanges() and getUserTrades() without currencyPair will return updates for ALL pairs.
}