...
public class CoinsphStreamingAccountService implements StreamingAccountService {
  ...
  public Observable<Balance> getBalanceChanges(Currency currency, Object... args) {
    ...
    // The listenKey subscribes to all user account updates. Filtering by currency happens client-side.
    // The channel name here might be a generic one like "balanceUpdate" or "outboundAccountPosition".
    
    // Ensure user data stream is started
    // service.startUserDataStreamIfNeeded(); // Example placeholder

    // String balanceUpdateChannel = "outboundAccountPosition"; // Or "balanceUpdate" - check Coins.ph docs
    // For now, let's assume a DTO CoinsphWebSocketBalanceUpdate and channel "balanceUpdate"
    // This part is highly dependent on the actual WebSocket message format from Coins.ph for balances.
    // If balance updates come via executionReport (e.g. for fees), that's handled by StreamingTradeService.
    // This method is for dedicated balance update messages.
    // The channel name "outboundAccountPosition" is based on Binance and Coins.ph user data stream docs.
    ...
    // This is handled by CoinsphStreamingService.subscribeChannel if isUserDataChannel returns true for "outboundAccountPosition".

    return service
        .subscribeChannel(balanceUpdateChannel) 
        .map(node -> mapper.treeToValue(node, CoinsphWebSocketOutboundAccountPosition.class))
        .flatMap(accountPosition -> Observable.fromIterable(CoinsphStreamingAdapters.adaptBalances(accountPosition)))
